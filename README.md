...
## Input
Most input files can be found in the `Data sources` folder, including the scraped KB linked data (created using the code in `KB linked data scraper`). The scraped landing pages are stored in `DBNL scraped` (created using the code in `DBNL scraper`).
